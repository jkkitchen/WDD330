...
export default class SwimmerProfiles {
    //Constructor
    constructor(swimmer, dataSource) {
    ...
    //Methods
    async init() {
        this.swimmers = await this.dataSource.findSwimmerById(this.swimmer);
    ...
    async addSwimmerToAccount(form) {
        ...
        swimmers.push(swimmerProfile);

        //Save swimmer to account
        setLocalStorage("swimmerData", swimmers);


        //Call swimmerProfileTemplate to display the new profile
        swimmerProfileTemplate(swimmerProfile);
    ...
    renderSwimmerProfile() {
        const swimmers = getLocalStorage("swimmerData") || [];
        swimmers.forEach(swimmer => swimmerProfileTemplate(swimmer));
    }
}
